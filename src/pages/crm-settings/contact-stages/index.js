...
import {
  ...
  clearMessages,
  deleteContactStage,
} from "../../../redux/contact-stages/"; // Redux actions and reducers for contact stages
...
const ContactStagesList = () => {
 ...
 const isAdmin = localStorage.getItem("role")?.includes("admin")
  ...
  const columns = [
    {
      ...
      dataIndex: "name",
      render: (text, record) => (
        <Link to={`/contact-stages/${record.id}`}>{record.name}</Link>
      ),
        ...
        <div className="dropdown table-action">
          ...
          <div className="dropdown-menu dropdown-menu-right">
            {isUpdate && <Link
              ...
              data-bs-target="#add_edit_contact_stage_modal"
              onClick={() => {
                setSelectedContactStage(record);
                setMode("edit");
              }}
            >
          ...
          {isDelete &&  <Link
              ...
              to="#"
              onClick={() => handleDeleteContactStage(record)}
            >
  ...
  const { contactStages, loading, error, success } = useSelector(
    (state) => state.contactStages
  );
  ...
  const handleDeleteContactStage = (contactStage) => {
    setSelectedContactStage(contactStage);
  ...
  return (
    <div className="page-wrapper">
              ...
              <div className="row align-items-center">
                <div className="col-8">
                  <h4 className="page-title">
                    Contact Stages
                    <span className="count-title">{contactStages?.length || 0}</span>
                  </h4>
                </div>
              ...
              <div className="card-header">
                <div className="row align-items-center">
                  <SearchBar
                    ...
                    label="Search Contact Stages"
                  />
                  {isCreate && <div className="col-sm-8">
                    <AddButton
                      label="Add Contact Stages"
                      id="add_edit_contact_stage_modal"
                      setMode={() => setMode("add")}
                    />
                  </div>}
                </div>
      ...
      <DeleteAlert
        ...
        selectedSource={selectedContactStage}
        onDelete={deleteData}
      />
    </div>
  );
...
export default ContactStagesList;
